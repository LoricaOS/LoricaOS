...
- When AI materially contributes to a commit, use a `Co-Authored-By` trailer
  that matches the actual model and harness doing the work. Use an identity
  supplied by that harness; if none is supplied, omit the trailer rather than
  inventing a model, vendor, or email address.
